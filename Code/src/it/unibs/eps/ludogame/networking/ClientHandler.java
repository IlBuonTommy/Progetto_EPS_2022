...
public class ClientHandler implements Runnable {
	...
	public void gestioneTurnoDue(int valoreDado) {
		...
		// mando alla view i bottoni che può abilitare QUESTO PASSAGGIO PUò ESSERE FATTO
		// IN AUTONOMIA DAL PROGRAMMA DEL GIOCATORE
	}

	// questa funzione viene richiamata dalla pressione del bottone da parte di un
	// giocatore
	public void gestioneTurnoTre(Posizione tastoPremuto) {
		// modifico il model in base al tasto premuto del giocatore
		model.tastoPremuto(tastoPremuto.getNomeposizione(), tastoPremuto.getColor(), tastoPremuto.getArrayposizione(),
				valoreDadoS);

		if (!model.tastoPremuto(tastoPremuto.getNomeposizione(), tastoPremuto.getArrayposizione(),
				tastoPremuto.getColor(), valoreDadoS)) {
	...
	public void gestioneTurnoQuattro() {
		if (model.checkWin() != -1) {
			...
			System.out.println("Partita terminata, Complimenti ha vinto il giocatore: "
					+ model.getPlayer()[model.checkWin()].getUsername());
			try {
				client.close();
			} catch (IOException e) {
